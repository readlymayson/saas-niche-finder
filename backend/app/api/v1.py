"""Internal API v1 — real endpoints backed by the ETL data pipeline."""
...
from app.db.vector import cosine_distance
...
@router.get(
    ...
    description=(
        ...
        '  "http://localhost:8000/v1/niches/top?limit=10&offset=0"\n'
        '```'
    ),
)
...
@router.get(
    ...
    description=(
        ...
        '```bash\n'
        'curl -H "X-Api-Token: <token>" \\\n'
        '  "http://localhost:8000/v1/niches/search?q=логистика&limit=10"\n'
        '```'
    ),
)
...
@router.get(
    ...
    description=(
        ...
        '```bash\n'
        'curl -H "X-Api-Token: <token>" \\\n'
        '  "http://localhost:8000/v1/niches/export?format=csv&category=saas"\n'
        '```'
    ),
)
...
@router.get(
    ...
    description=(
        ...
        '```bash\n'
        'curl -H "X-Api-Token: <token>" \\\n'
        '  "http://localhost:8000/v1/niches/42"\n'
        '```'
    ),
)
...
@router.get(
    ...
    description=(
        ...
        '```bash\n'
        'curl -H "X-Api-Token: <token>" \\\n'
        '  "http://localhost:8000/v1/niches/42/similar?limit=5"\n'
        '```'
    ),
)
